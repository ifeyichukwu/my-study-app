  ...
  useEffect(() => {
    if (!currentSessionId) {
      setMessages([
        {
          id: '1',
          content: `Hello! I'm LearnMi, your AI Learning Assistant. I'm here to help you understand concepts, answer questions, and support your academic journey.

I can help you with:
• 📚 Explaining complex topics and concepts
...
• 💡 Providing examples and analogies to clarify ideas
• 📖 Suggesting additional resources for deeper learning
• ✏️ Helping with assignments and homework guidance
...
What would you like to learn about today? Feel free to ask me anything related to your studies!`,
  ...
  const handleSendMessage = async () => {
    ...
    if (!sessionId && user) {
      try {
        const newSession = await createSession.mutateAsync('LearnMi Chat');
    ...
    try {
      const context = {
        profile: profile,
        userMessage: `As LearnMi, the learning assistant specialist: ${inputValue}`,      
      ...
      if (sessionId && user) {
        updateSession.mutate({ 
          ...
          title: finalMessages.length === 2 ? `LearnMi: ${inputValue.slice(0, 40)}...` : undefined
        });
      }
    } catch (error) {
  ...
  const handleShareSession = (sessionId: string) => {
    navigator.clipboard.writeText(`LearnMi Session: ${sessionId}`);
  };
    ...
    <div className="w-64 bg-sidebar border-r border-sidebar-border h-screen flex flex-col">
      ...
      <div className="p-4 border-b border-sidebar-border">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-semibold text-sidebar-foreground">LearnMi</span>
        </div>
        <Button
      ...
      <ScrollArea className="flex-1 p-2">
        <div className="space-y-1">
          {sessions.filter(s => s.title.includes('LearnMi')).map((session) => (
            <div
              key={session.id}
          ...
          <div className="h-14 border-b border-border flex items-center px-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-semibold text-foreground">
                LearnMi - Your Learning Assistant
              </h1>
            </div>
            {!user && (
              <div className="ml-auto text-sm text-muted-foreground">
                <span>Sign in to save learning sessions</span>
              </div>
            )}
          </div>
          ...
          <div className="p-4 border-t border-border">
            <div className="max-w-3xl mx-auto flex gap-2">
              <Input
                ...
                placeholder="Ask LearnMi to explain concepts, answer questions, create study materials..."
                className="flex-1"
                disabled={isLoading}
              />
              <Button
